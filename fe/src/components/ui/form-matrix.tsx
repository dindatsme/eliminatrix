...
export function FormMatrix({ onCalculate }: {
  ...
  const [tooltipTarget, setTooltipTarget] = useState<{ row: number; col: number } | null>(null)
  ...
  return (
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 font-orienta">
        <div className="flex flex-wrap gap-4">
          <FormLabel className="font-libre-franklin font-medium">Ukuran Matrix (Augmented):</FormLabel>
          <div className="flex flex-wrap gap-4">
            <FormField
            control={control}
            name="rows"
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription className="text-center text-muted-col text-xs">Baris (1 s.d. 5)</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription className="text-center text-muted-col text-xs">Kolom (2 s.d. 6)</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
        ...
        {rows > 0 && cols > 0 && rows <= 5 && cols <= 6 && (
          <div className="flex flex-col md:flex-row gap-4 space-y-2 items-start w-full">
            <FormLabel className="shrink-0 font-libre-franklin font-medium">Input Matriks:</FormLabel>
            <div className="grid w-full gap-3" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
              {matrix.map((row, rIdx) =>
                row.map((_value, cIdx) => (
                  
                  <Tooltip key={`${rIdx}-${cIdx}`} open={tooltipTarget?.row === rIdx && tooltipTarget?.col === cIdx}>
                    <TooltipTrigger asChild>
                      ...
                      onKeyDown={(e) => {
                        if(e.key === ",") {
                          ...
                          setTimeout(() => setTooltipTarget(null), 2000)
                    ...
                    <TooltipContent side="top">
                      Gunakan titik (.) untuk desimal, bukan (,)
                    </TooltipContent>
                  </Tooltip>
                ))
              )}
        ...
        <div className="flex flex-wrap">
          <FormLabel className="font-libre-franklin font-medium">Metode:</FormLabel>
          <FormField
            control={control}
            name="method"
            render={({ field }) => (
                ...
                <FormControl>
                  ...
                  >
                    <FormItem className="flex items-center space-x-0.5">
                      ...
                      <FormLabel className="text-xs font-normal">Gauss</FormLabel>
                    </FormItem>
                    <FormItem className="flex items-center space-x-0.5">
                      <FormControl>
                        <RadioGroupItem value="gauss-jordan" />
                      </FormControl>
                      <FormLabel className="text-xs font-normal">Gauss-Jordan</FormLabel>
                    </FormItem>
                  </RadioGroup>
                </FormControl>
                <FormMessage />
        ...
        <div className="flex gap-4">
          <Button type="submit" className="font-bold font-libre-franklin">Hitung!</Button>
          <Button 
            type="button" 
            variant="secondary" 
